...
def init_app():
    ...
    # ✅ 3. 세션 상태 기본값 설정
    st.session_state.setdefault("selected_article", None)   # 선택된 뉴스 기사
    st.session_state.setdefault("chat_history", [])         # 챗봇 대화 기록
    st.session_state.setdefault("term_click_count", 0)      # 금융 용어 클릭 수
    st.session_state.setdefault("news_click_count", 0)      # 뉴스 클릭 수
    st.session_state.setdefault("chat_count", 0)            # 챗봇 대화 수
